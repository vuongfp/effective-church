        ...
        >
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">{isEdit ? "Edit Group" : "New Group"}</h2>
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                    ...
                    ${form.type === t ? "bg-indigo-600 text-white border-indigo-600" : "border-slate-200 text-slate-500 hover:border-indigo-300"}`}>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {selectedMembers.map(m => (
                                    <span key={m.id} className="flex items-center gap-1 text-xs bg-indigo-100 text-indigo-700 px-2.5 py-1 rounded-full font-medium">
                                        ...
                                        <button type="button" onClick={() => toggleMember(m.id)} className="ml-0.5 hover:text-red-600 transition-colors">
                                            <X className="w-3 h-3" />
                                        </button>
                                    </span>
                                ))}
                            ...
                            {loadingMembers ? (
                                ...
                                <div className="max-h-52 overflow-y-auto divide-y divide-slate-50">
                                    {filteredMembers.length === 0 ? (
                                        <p className="text-center text-xs text-slate-400 py-6">No members found</p>
...
/* ── Main GroupsTab Component ─────────────────────────────────────── */
interface GroupsTabProps {
    members: any[];
}

export default function GroupsTab({ members }: GroupsTabProps) {
    const supabase = createClient();
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex gap-3 text-xs text-slate-500 flex-wrap">
                <span className="bg-slate-100 px-3 py-1 rounded-full">{groups.length} groups total</span>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-3">
                    ...
                    <p className="text-sm font-medium">{search ? "No groups match your search." : "No groups yet."}</p>
                    {!search && <button onClick={() => setModalGroup({})} className="mt-1 text-sm text-indigo-600 hover:underline">Create the first group</button>}
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map(g => (
                        <div key={g.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                            ...
                            <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between">
                                <span className="text-xs text-slate-400 flex items-center gap-1">
                                    <Users className="w-3.5 h-3.5" /> {g.member_count ?? 0} members
                                </span>
